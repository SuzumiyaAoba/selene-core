...
- [x] `syntax-rules` マクロシステム
- [x] 数値塔の拡張 (有理数, 複素数)
- [x] ポート操作 (`open-input-file`, `open-output-file`, `read-char`, `peek-char`, `write-char`, `eof-object?`, etc.)
- [x] `load` 関数
...
- [x] IR 最適化 - 不要コード除去
- [x] IR 最適化 - インライン展開
- [x] バイトコードのシリアライズ/デシリアライズ
  - [x] シリアライズ（テキスト形式）
  - [x] デシリアライズ（テキスト形式）
  - [x] バイナリ形式（シリアライズ/デシリアライズ）

### Phase 6: 仮想マシン (VM)

- [x] VM アーキテクチャ設計
  - [x] スタックベース vs レジスタベース
  - [x] 命令セットの定義
  - [x] メモリモデル
- [x] 基本命令の実装
  - [x] スタック操作 (push, pop, dup)
  - [x] 算術命令
  - [x] 分岐命令 (jump, branch)
  - [x] 関数呼び出し (call, return)
- [x] クロージャのサポート
- [x] 継続のサポート
- [x] ガベージコレクション
- [x] 末尾呼び出し最適化
...
- [x] REPL の VM 対応
- [x] クロージャの自由変数キャプチャ（参照キャプチャ、ボックス化対応）
- [x] デバッガ
- [x] プロファイラ
- [ ] バイトコードダンプツール
...
├── main.mbt               # エントリポイント
├── ir.mbt                 # 中間表現 (IR)
├── compile.mbt            # AST → IR コンパイラ
├── opcode.mbt             # バイトコード定義
├── codegen.mbt            # IR → バイトコード生成
├── optimize.mbt           # IR 最適化
├── vm.mbt                 # 仮想マシン
├── vm_repl.mbt            # VM REPL コンテキスト
├── gc.mbt                 # ガベージコレクション
├── debugger.mbt           # デバッガ
├── profiler.mbt           # プロファイラ
└── *_test.mbt             # テストファイル群
