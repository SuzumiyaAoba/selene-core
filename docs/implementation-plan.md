...
## 直近の着手タスク（スプリント候補）
- [x] 字句解析器スケルトンと最小テスト（識別子/数/括弧/コメント）
- [x] クォート展開（`'`/`` ` ``/`,`/`,@`）の AST 変換ユーティリティ
- [x] パーサの骨組み（S 式 → AST）とエラーレポート整備
- [x] 値型の定義ドラフト（`src/lang/value.mbt` 等）と環境構造の設計メモ
- [x] REPL/評価器用のエントリポイント雛形（`src/repl/main.mbt` 想定）
